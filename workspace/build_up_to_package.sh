...
while (($#)); do case "$1" in
  ...
  -h|--help) echo '用法：build_up_to_package.sh --package NAME [--workspace PATH] [-- 额外 colcon 参数]'; exit 0;;
  --) shift; ARGS+=("$@"); break;; *) echo "未知参数：$1" >&2; exit 2;; esac; done
[[ -n "$PACKAGE" ]] || { echo '缺少 --package' >&2; exit 2; }
source "/opt/ros/${ROS_DISTRO:-jazzy}/setup.bash"
exec colcon build --base-paths "$WORKSPACE/src" --packages-up-to "$PACKAGE" --symlink-install "${ARGS[@]}"

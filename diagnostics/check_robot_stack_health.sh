...
while (($#)); do case "$1" in --joint-states) JOINT_STATES="${2:?}"; shift 2;; --move-group) MOVE_GROUP="${2:?}"; shift 2;; --manager) MANAGER="${2:?}"; shift 2;; -h|--help) echo '用法：check_robot_stack_health.sh [--joint-states TOPIC] [--move-group NODE] [--manager PATH]'; exit 0;; *) echo "未知参数：$1" >&2; exit 2;; esac; done
